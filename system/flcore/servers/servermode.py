...
import time
from flcore.clients.clientmode import clientMODE
...
class MODE(Server):
    ...
    def unlearn(self):
        ...
        for i in range(self.global_rounds + 1):
            ...
            self.selected_clients = self.select_clients()
            """
                This part supports the Alg. 1 ~ line 5.
                    - self.send_models() send M to all clients.
                    - self.send_de_models() send M_de (randomly initialized parameters 
                        // others like computation graphs and optimizers are saved?) to remaining clients
            """
            self.send_models()
            self.send_de_models()
            ...
            if i < self.args.r_de:
                # Loops over remaining clients only
                for client in self.selected_clients:
                    client.unlearn()
                # Receive the models from remaining clients
                self.receive_models()
            ...
            for client in self.selected_clients:
                client.train()
            # Receive the models from all clients
            self.receive_models()
            if self.dlg_eval and i % self.dlg_gap == 0:
                self.call_dlg(i)
            """
                Eq. (4) goes here: aggregate instead of self.aggregate_parameters()
                - self.aggregate_mode()
            """
            self.aggregate_parameters()
