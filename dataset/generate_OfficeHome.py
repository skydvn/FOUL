...
def download_OfficeHome_dataset():
    ...
    download_and_extract("https://drive.google.com/uc?id=1uY0pj7oFsjMxRwaD3Sxy0jgel0fsYXLC",
                         os.path.join(data_path+"rawdata/", "office_home.zip"))


def read_OfficeHome_data(dataset_path, domain_name, split="train"):
    data_paths = []
    data_labels = []
    split_file = path.join(dataset_path, "splits", "{}_{}.txt".format(domain_name, split))
    with open(split_file, "r") as f:
...
def get_OfficeHome_dloader(dataset_path, domain_name):
    train_data_paths, train_data_labels = read_OfficeHome_data(dataset_path, domain_name, split="train")
    test_data_paths, test_data_labels = read_OfficeHome_data(dataset_path, domain_name, split="test")
    transforms_train = transforms.Compose([
        transforms.RandomResizedCrop(64, scale=(0.75, 1)),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor()
...
def generate_dataset(dir_path, num_clients, niid, balance, partition):
    ...
    for id, d in enumerate(domains):
        train_loader, test_loader = get_OfficeHome_dloader(root, d)
